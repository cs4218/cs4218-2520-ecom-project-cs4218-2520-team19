...
//payment gateway
//Sun Zhiyuan Felix (A0272474Y) - change gateway initialization to a function for testing
let gateway;
...
export const productPhotoController = async (req, res) => {
  try {
    ...
    if (product.photo.data) {
      ...
      return res.status(404).send({ // Sun Zhiyuan Felix (A0272474Y) - return 404 if photo not found
        success: false,
...
export const brainTreeTokenController = async (req, res) => {
  try {
    ...
    activeGateway.clientToken.generate({}, function (err, response) {
      if (err) {
...
export const brainTreePaymentController = async (req, res) => {
  try {
    ...
    let newTransaction = activeGateway.transaction.sale(
      {
        amount: total,
